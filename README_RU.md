...
* **Строгое разделение Model/UI:** Ядро (`bdoc-model`) состоит из POJO, отвязанных от UI, что позволяет работать в headless-режиме.
* **Модульная плагинная архитектура (ModuleLayer):** Динамическое сканирование `plugins/` при старте.
* **Гибридный стриминг (JSON + CBOR):** Экономия RAM за счет ленивой загрузки тяжелых страниц (`loadPage`) через `CBORMapper`.
* **Асинхронный DTP-движок (`TaskQueue`):** Защита JavaFX от фризов при тяжелых операциях.
